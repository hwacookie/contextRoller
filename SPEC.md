...
| `/contextRoller help` | Command reference + short "how it works" (Markdown overlay in TUI, plain text elsewhere) |
...
`~/.pi/agent/contextRoller.json` (global, per-user default — same layering as pi's own model settings) with `.pi/contextRoller.json` (project) overriding per key:
...
If no secondary model is resolvable from either file, the extension stays inactive and notifies once at session start: *"no secondary model configured — run /contextRoller model to pick one"*.
...
**Baseline backfill.** When a project is opened whose today-file has no entries yet and the (resumed) session already contains substantive content, the extension asks once (TUI only, deferred until after startup): *"Create a baseline entry from this session's existing context?"* On yes, one secondary-model call writes an entry with a **single timestamp** (the moment of writing — earlier work has no precise timestamps). Source: the rolling summary when available (already condensed), else a capped raw serialization of the branch (tail = most recent work). Non-TUI modes never prompt.
...
- [x] Diary baseline backfill (§5.7): on session start, when today's diary file is empty and the resumed session has substantive content, a TUI confirm dialog offers to write a baseline entry from the existing session context (rolling summary if present, else capped raw branch serialization) with a single-timestamp header; non-TUI modes never prompt; no re-prompt once today's file has entries. Verified via PTY: dialog appears on resume of a content-bearing session, Enter/Yes writes a correct entry (`## date HH:MM (user)`), and a second launch with the file present shows no dialog
- [x] TUI-quiet diagnostics: all `[contextRoller]` stderr lines are suppressed in TUI mode (the terminal belongs to the TUI — raw writes corrupted the input area; the status line covers state there). Non-TUI modes (RPC/print) keep stderr logging, which the RPC test clients rely on (`idle`, `diary entry written` markers)
- [x] Missing-model notice (§5.5): when no secondary model is resolvable from global or project config, session start notifies once ("run /contextRoller model to pick one") instead of idling silently; §5.5 merge direction also corrected (global default, project overrides)
